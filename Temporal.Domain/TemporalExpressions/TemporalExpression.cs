...
namespace Temporal.Domain.TemporalExpressions
{
    ...
    public abstract class TemporalExpression
    {
        ...
        {
            if (GetType() == typeof(TEUnion))
            {
                ((TEUnion) this).Add(expr);
                return this;
            }

            var union = new TEUnion();
            union.Add(this);
            union.Add(expr);
            return union;
        }
        ...
        {
            if (GetType() == typeof(TEIntersect))
            {
                ((TEIntersect) this).Add(expr);
                return this;
            }

            var intersect = new TEIntersect();
            intersect.Add(this);
            intersect.Add(expr);
            return intersect;
        }
        ...
        {
            return Occurrences(rangeStart, rangeEnd, 0, precision);
        }
        ...
        ///     Max number of occurrences to return. Default 0 means infinte.
        ///     rangeEnd must be set if maxOccurrences is 0.
        /// </param>
        /// <param name="precision">
        ///     For performance use the highest value possible. If using time temporal
        ///     expressions (hour,min,sec) increment should be set accordingly. Valid values are
        ///     Days, Hours, Mins, Secs, Weeks.
        /// </param>
        /// <returns></returns>
        public List<DateTime> Occurrences(DateTime rangeStart, DateTime? rangeEnd, int maxOccurrences,
            IntervalPrecision precision)
        {
            ...
            var counter = new DateTime(rangeStart.Ticks);
